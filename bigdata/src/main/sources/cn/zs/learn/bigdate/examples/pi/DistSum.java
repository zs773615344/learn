 ...
 */
package cn.zs.learn.bigdate.examples.pi;
...
import cn.zs.learn.bigdate.examples.pi.Container;
import cn.zs.learn.bigdate.examples.pi.SummationWritable;
import cn.zs.learn.bigdate.examples.pi.TaskResult;
import cn.zs.learn.bigdate.examples.pi.Util;
import cn.zs.learn.bigdate.examples.pi.math.Summation;
...
public final class DistSum extends Configured implements Tool {
  ...
  static class Parameters {
    ...
    static Parameters parse(String[] args, int i) {
      ...
      cn.zs.learn.bigdate.examples.pi.Util.checkDirectory(localDir);
      ...
      try {
        for(;; Thread.sleep(2000)) {
          ...
          if (m >= parts || r >= parts) {
            ...
            cn.zs.learn.bigdate.examples.pi.Util.out.println("  " + this + " is " + value + " (m=" + m + ", r=" + r + ")");
            return value;
          }
        }
  ...
  /////////////////////////////////////////////////////////////////////////////
  private final cn.zs.learn.bigdate.examples.pi.Util.Timer timer = new cn.zs.learn.bigdate.examples.pi.Util.Timer(true);
  ...
  private void compute(final String name, Summation sigma) throws IOException {
    ...
    if (!cn.zs.learn.bigdate.examples.pi.Util.createNonexistingDirectory(fs, dir))
      return;
    ...
    final String startmessage = "steps/parts = "
      ...
      + " = " + cn.zs.learn.bigdate.examples.pi.Util.long2string(sigma.E.getSteps()/parameters.nParts);
    cn.zs.learn.bigdate.examples.pi.Util.runJob(name, job, parameters.machine, startmessage, timer);
    final List<TaskResult> results = cn.zs.learn.bigdate.examples.pi.Util.readJobOutputs(fs, outdir);
    cn.zs.learn.bigdate.examples.pi.Util.writeResults(name, results, fs, parameters.remoteDir);
    fs.delete(dir, true);

    //combine results
    final List<TaskResult> combined = cn.zs.learn.bigdate.examples.pi.Util.combine(results);
    final PrintWriter out = cn.zs.learn.bigdate.examples.pi.Util.createWriter(parameters.localDir, name);
    try {
      for(TaskResult r : combined) {
        final String s = taskResult2string(name, r);
        out.println(s);
        out.flush();
        cn.zs.learn.bigdate.examples.pi.Util.out.println(s);
      }
    } finally {
      out.close();
    }
    ...
    public Computation call() {
      if (sigma.getValue() == null)
        try {
          ...
          cn.zs.learn.bigdate.examples.pi.Util.out.println("ERROR: Got an exception from " + getJobName());
          e.printStackTrace(cn.zs.learn.bigdate.examples.pi.Util.out);
        }
      return this;
    }
  }
  ...
  private Summation execute(String name, Summation sigma) {
    ...
    try {
      cn.zs.learn.bigdate.examples.pi.Util.execute(parameters.nThreads, computations);
    } catch (Exception e) {
      throw new RuntimeException(e);
    }

    final List<Summation> combined = cn.zs.learn.bigdate.examples.pi.Util.combine(Arrays.asList(summations));
  ...
  public int run(String[] args) throws Exception {
    ...
    if (args.length != Parameters.COUNT + 2)
      return cn.zs.learn.bigdate.examples.pi.Util.printUsage(args, getClass().getName()
    ...
    cn.zs.learn.bigdate.examples.pi.Util.out.println();
    cn.zs.learn.bigdate.examples.pi.Util.out.println("name  = " + name);
    cn.zs.learn.bigdate.examples.pi.Util.out.println("sigma = " + sigma);
    cn.zs.learn.bigdate.examples.pi.Util.out.println(parameters);
    Util.out.println();
