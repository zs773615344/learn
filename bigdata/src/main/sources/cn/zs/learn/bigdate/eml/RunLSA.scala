package cn.zs.learn.bigdate.eml
...
//    val numTerms = if (args.length > 1) args(1).toInt else 20000
    val spark = SparkSession.builder().appName("基于维基百科的数据分析").master("").config("spark.serializer",classOf[KryoSerializable].getName).getOrCreate()
    ...
    val (docTermMatrix, termIds, docIds, termIdfs) = documentTermMatrix(docTexts, "hdfs://10.11.100.102:9000/stopwords.txt", numTerms)
    //val matrix: DataFrame = docTermMatrix.cache()
   // val vecRdd: RDD[MLLibVector] = docTermMatrix.select("tfidfVec").rdd.map(row=>Vectors.fromML(row.getAs[MLVector]("tfidfVec")))
