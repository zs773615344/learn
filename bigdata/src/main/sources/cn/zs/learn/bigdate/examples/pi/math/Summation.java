...
import cn.zs.learn.bigdate.examples.pi.math.ArithmeticProgression;
import cn.zs.learn.bigdate.examples.pi.math.Modular;
import cn.zs.learn.bigdate.examples.pi.math.Montgomery;
...
public class Summation implements Container<Summation>, Combinable<Summation> {
  ...
  /** Compute the value using {@link cn.zs.learn.bigdate.examples.pi.math.Modular#mod(long, long)}. */
  double compute_modular() {
    long e = E.value;
    long n = N.value;
    double s = 0;
    for(; e > E.limit; e += E.delta) {
      s = cn.zs.learn.bigdate.examples.pi.math.Modular.addMod(s, cn.zs.learn.bigdate.examples.pi.math.Modular.mod(e, n)/(double)n);
      n += N.delta;
    }
    return s;
  }
  ...
  double compute_montgomery() {
    ...
    double s = 0;
    for(; e > E.limit; e += E.delta) {
      s = cn.zs.learn.bigdate.examples.pi.math.Modular.addMod(s, montgomery.set(n).mod(e)/(double)n);
      n += N.delta;
    }
    return s;
  }
